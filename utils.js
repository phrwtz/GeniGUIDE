...
function filterActionsByStudentAndActivity(student, activity) {
    var arr = [],
        act;
    if (student.actions) {
        for (var i = 0; i < student.actions.length; i++) {
        ...
        switch (act.event) {
            ...
            case 'Guide remediation requested':
                ...
                case 'Challenge retried':
                    ...
                    break;
            case 'Drake submitted':
                excessChanges = Math.max(0, (changes - minimumChanges));
                outcomeStr = (act.parameters.correct ? "Correct" : "Incorrect");
                profile = outcomeStr + " drake submitted after " + changes + " changes, " + excessChanges + " more than minimally needed. " + hints + " hints received, " + remediations + " remediations received. ";
                if (retries > 0) {
